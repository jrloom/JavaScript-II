...
let allCaps = [];
runners.map(name => allCaps.push(name.first_name.toUpperCase()));
console.log(allCaps);
...
let largeShirts = runners.filter(info => info.shirt_size === "L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
// let ticketPriceTotal = [];
let ticketPriceTotal = runners.reduce((acc, currentValue) => (acc += currentValue.donation), 0);
...
// Problem 1
let mailingList = runners.map(spam => spam.email);
console.log(mailingList);

// Problem 2
let topTier = runners.filter(donors => donors.donation > 200);
console.log(topTier);

// Problem 3
let companies = runners.map(company => company.company_name);
let companyAlphabetical = companies.sort();
console.log(`${companyAlphabetical}`);
